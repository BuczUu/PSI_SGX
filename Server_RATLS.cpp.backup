...
void *client_handler(void *arg)
{
    ...
    /* ===== MUTUAL ATTESTATION: Serwer wykonuje RA wobec klienta ===== */
    printf("[SERVER] Starting mutual attestation - proving server enclave to client %d...\n", client->client_id);

    // Inicjalizuj drugi kontekst RA dla serwera (jako initator wobec klienta-SP)
    sgx_ra_context_t server_ra_context = 0;
    sgx_status_t server_ra_status = SGX_ERROR_UNEXPECTED;
    sgx_status_t server_ra_ret = enclave_init_ra(global_eid, &server_ra_status, 0, &server_ra_context);
    if (server_ra_ret != SGX_SUCCESS || server_ra_status != SGX_SUCCESS)
    {
        printf("[SERVER] Server RA init failed: ret=0x%x, ret_status=0x%x\n", server_ra_ret, server_ra_status);
        enclave_ra_close(global_eid, &ret_status, ra_context);
        close(client->socket);
        free(client);
        return NULL;
    ...
    // Wygeneruj MSG1 serwera
    sgx_ra_msg1_t server_msg1;
    memset(&server_msg1, 0, sizeof(server_msg1));
    server_ra_ret = sgx_ra_get_msg1(server_ra_context, global_eid, sgx_ra_get_ga, &server_msg1);
    if (server_ra_ret != SGX_SUCCESS)
    {
        printf("[SERVER] Failed to generate server MSG1: 0x%x\n", server_ra_ret);
        enclave_ra_close(global_eid, &server_ra_status, server_ra_context);
        enclave_ra_close(global_eid, &ret_status, ra_context);
        close(client->socket);
        free(client);
        return NULL;
    ...
    // Wyslij MSG1 do klienta
    if (send(client->socket, &server_msg1, sizeof(server_msg1), 0) < 0)
    {
        printf("[SERVER] Failed to send server MSG1 to client %d\n", client->client_id);
        enclave_ra_close(global_eid, &server_ra_status, server_ra_context);
        enclave_ra_close(global_eid, &ret_status, ra_context);
        close(client->socket);
        free(client);
    ...
    printf("[SERVER] Server MSG1 sent to client %d\n", client->client_id);

    // Odbierz MSG2 od klienta (klient dziala jako SP)
    uint32_t server_msg2_size;
    if (recv(client->socket, &server_msg2_size, sizeof(server_msg2_size), 0) <= 0)
    {
        printf("[SERVER] Failed to receive server MSG2 size from client %d\n", client->client_id);
        enclave_ra_close(global_eid, &server_ra_status, server_ra_context);
        enclave_ra_close(global_eid, &ret_status, ra_context);
        close(client->socket);
        free(client);
    ...
    sgx_ra_msg2_t *p_server_msg2 = (sgx_ra_msg2_t *)malloc(server_msg2_size);
    if (!p_server_msg2)
    {
        printf("[SERVER] Memory allocation failed for server MSG2\n");
        enclave_ra_close(global_eid, &server_ra_status, server_ra_context);
        enclave_ra_close(global_eid, &ret_status, ra_context);
        close(client->socket);
        free(client);
    ...
    if (recv(client->socket, p_server_msg2, server_msg2_size, 0) <= 0)
    {
        printf("[SERVER] Failed to receive server MSG2 from client %d\n", client->client_id);
        free(p_server_msg2);
        enclave_ra_close(global_eid, &server_ra_status, server_ra_context);
        enclave_ra_close(global_eid, &ret_status, ra_context);
        close(client->socket);
    ...
    printf("[SERVER] Server MSG2 received from client %d (size: %u)\n", client->client_id, server_msg2_size);

    // Przetworzenie MSG2 i generowanie MSG3
    sgx_ra_msg3_t *p_server_msg3 = NULL;
    uint32_t server_msg3_size = 0;
    server_ra_ret = sgx_ra_proc_msg2(server_ra_context, global_eid, sgx_ra_proc_msg2_trusted,
                                     sgx_ra_get_msg3_trusted, p_server_msg2, server_msg2_size,
                                     &p_server_msg3, &server_msg3_size);
    free(p_server_msg2);

    if (server_ra_ret != SGX_SUCCESS || !p_server_msg3)
    {
        printf("[SERVER] Failed to process server MSG2: ret=0x%x\n", server_ra_ret);
        if (p_server_msg3)
            free(p_server_msg3);
        enclave_ra_close(global_eid, &server_ra_status, server_ra_context);
        enclave_ra_close(global_eid, &ret_status, ra_context);
    ...
    // Wyslij MSG3 do klienta
    if (send(client->socket, &server_msg3_size, sizeof(server_msg3_size), 0) < 0 ||
        send(client->socket, p_server_msg3, server_msg3_size, 0) < 0)
    {
        printf("[SERVER] Failed to send server MSG3 to client %d\n", client->client_id);
        free(p_server_msg3);
        enclave_ra_close(global_eid, &server_ra_status, server_ra_context);
        enclave_ra_close(global_eid, &ret_status, ra_context);
        close(client->socket);
    ...
    free(p_server_msg3);
    printf("[SERVER] Server MSG3 sent to client %d\n", client->client_id);

    // Odbierz wynik weryfikacji od klienta
    uint32_t server_attestation_result;
    if (recv(client->socket, &server_attestation_result, sizeof(server_attestation_result), 0) <= 0)
    {
        printf("[SERVER] Failed to receive server attestation result from client %d\n", client->client_id);
        enclave_ra_close(global_eid, &server_ra_status, server_ra_context);
        enclave_ra_close(global_eid, &ret_status, ra_context);
        close(client->socket);
        free(client);
    ...
    if (server_attestation_result != 0x00000000)
    {
        printf("[SERVER] Client rejected server attestation!\n");
        enclave_ra_close(global_eid, &server_ra_status, server_ra_context);
        enclave_ra_close(global_eid, &ret_status, ra_context);
        close(client->socket);
        free(client);
    ...
    printf("[SERVER] ===== MUTUAL ATTESTATION COMPLETE for client %d - both enclaves verified! =====\n", client->client_id);

    // Zamknij server RA context (nie jest juz potrzebny)
    enclave_ra_close(global_eid, &server_ra_status, server_ra_context);
